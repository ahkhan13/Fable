    ...
    if(mysqli_num_rows($result)>0){
     ...
     <span>
      <?php echo "<button data-id='{$sender_id}' 
    data-sender='{$sender}'   class='btn btn-primary btn-block btn-sm'  id='remove_friend'>Remove Friend</button>"; ?>
                                            </span>  
                                    </div>
                <?php } 
        } else{
...
$('#already').removeClass('already_requested').addClass('already_friend').html('Removed Successfuly').fadeIn();
                    }else{
                        alert('try again');
                    }
                }
